...
Inactive titles have a grey (~#bebebe~) foreground and a very dark grey (~#404040~) background color. Active titles have a white (~#ffffff~) foreground and a steel blue (~#4682b4~) background color. Both use transparency and are tinted with 50% of the respective background color:
#+begin_src conf-space
  Colorset 1 fg Grey, bg Grey25, RootTransparent, Tint Grey25 50
  Colorset 2 fg White, bg SteelBlue, RootTransparent, Tint SteelBlue 50
#+end_src
...
#+begin_src conf-space
  Colorset 3 fg Grey, bg Grey25, DGradient 64 Grey25 Black
  Colorset 4 fg White, bg SteelBlue, DGradient 64 SteelBlue Black
#+end_src

** Window Styles
In general, colorset 1 is used as default (inactive) and colorset 2 for hilighted active items. This is necessary for title or icon texts:
#+begin_src conf-space
...
*** Button styles
Buttons in general use the same style (colorsets) like titles. They are flat and have no reliefs as well. Button 1 shows the mini icon of the current application, if defined:
#+begin_src conf-space
