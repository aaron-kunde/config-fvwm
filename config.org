...
** Colors
The default colorset is colorset 1, which uses the transparent grey background of the pager, like inactive window titles. The active desktop is hilighted with colorset 2 like active window titles. Window whithin the pager use the same colorsets as the borders for acitve and inactive windows. Balloon texts use the same colorset like active menuitems:
#+begin_src conf-space
